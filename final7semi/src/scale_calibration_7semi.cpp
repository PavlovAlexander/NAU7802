...
bool collectPoint(NAU7802_7Semi& scale, float targetWeight, CalibrationPoint& outPoint) {
    ...
    snprintf(msgSetup,
             sizeof(msgSetup),
             "readAverage N=%u T=%lu ms",
             static_cast<unsigned>(kCalWizardSamples),
             static_cast<unsigned long>(kCalWizardTimeoutMs));
    printTagged("CALIB", msgSetup);

    int32_t avgRaw = 0;
    if (!scale.readAverage(avgRaw, kCalWizardSamples, kCalWizardTimeoutMs)) {
